            ...
            <div class="row g-3">
                ...
                <div class="col-md-6">
                    ...
                    <select name="subject_id" class="form-select @error('subject_id') is-invalid @enderror" required>
                        ...
                        @foreach($subjects as $sub)
                            <option value="{{ $sub->id }}" {{ $schedule->subject_id==$sub->id?'selected':'' }}>{{ $sub->name }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="col-md-6">
                    ...
                    <select name="hall_id" class="form-select @error('hall_id') is-invalid @enderror" required>
                        ...
                        @foreach($halls as $h)
                            <option value="{{ $h->id }}" {{ $schedule->hall_id==$h->id?'selected':'' }}>{{ $h->name }} ({{ $h->type === 'lab' ? 'معمل' : 'مدرج' }})</option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-6">
                    ...
                    <select name="student_group_id" class="form-select @error('student_group_id') is-invalid @enderror" required>
                        ...
                        @foreach($studentGroups as $g)
                            <option value="{{ $g->id }}" {{ $schedule->student_group_id==$g->id?'selected':'' }}>{{ $g->name }}</option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-4">
                    ...
                    <select name="day_of_week" class="form-select @error('day_of_week') is-invalid @enderror" required>
                        ...
                        <option value="wednesday" {{ $schedule->day_of_week=='wednesday'?'selected':'' }}>الأربعاء</option>
                ...
                <div class="col-md-6">
                    ...
                    <select name="type" class="form-select @error('type') is-invalid @enderror">
                        <option value="lecture" {{ $schedule->type === 'lecture' ? 'selected' : '' }}>محاضرة نظرية</option>
